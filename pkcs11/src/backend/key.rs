...
impl NetHSMId {
    ...
    fn validate(s: &str) -> Result<(), InvalidIdError> {
        ...
        if let Some(c) = rest
            .iter()
            .find(|c| !c.is_ascii_alphanumeric() && !Self::ALLOWED_CHARS.contains(c))
        {
    ...
    fn from_unencoded_bytes(bytes: &[u8]) -> Result<Self, InvalidIdError> {
        let s = str::from_utf8(bytes).map_err(|_| {
            warn!("ID {:?} is invalid: not a UTF-8 string", bytes);
    ...
    fn from_encoded_bytes(bytes: &[u8]) -> Result<Self, InvalidIdError> {
        let max_len = (Self::MAX_LEN - Self::ENCODED_PREFIX.len()) / 2;
        if bytes.len() > max_len {
            warn!(
                "ID {bytes:?} is invalid: length is {} bytes (maximum for encoded IDs: {} bytes)",
                bytes.len(),
                max_len,
            );
            Err(InvalidIdError::TooLong)
        } else {
            let mut s = Self::ENCODED_PREFIX.to_owned();
    ...
    fn decode(&self) -> Option<Vec<u8>> {
        let rest = self.0.strip_prefix(Self::ENCODED_PREFIX)?;
...
impl TryFrom<&Pkcs11Id<'_>> for NetHSMId {
    ...
    fn try_from(id: &Pkcs11Id<'_>) -> Result<Self, Self::Error> {
        let id = id.0.as_ref();
        Self::from_unencoded_bytes(id).or_else(|err| {
            if err == InvalidIdError::InvalidCharacter {
                Self::from_encoded_bytes(id)
            } else {
                Err(err)
            }
        })
...
mod tests {
    ...
    fn test_nethsm_id_valid() {
        let valid_ids: [(&[u8], &str); _] = [
            ...
            (b"&*&*&*", "0-----262A262A262A"),
            (b"--", "0-----2D2D"),
            (b".234567890123456789012345678901234567890123456789012345678901", "0-----2E323334353637383930313233343536373839303132333435363738393031323334353637383930313233343536373839303132333435363738393031"),
            (&[0], "0-----00"),
    ...
    fn test_nethsm_id_invalid() {
        let invalid_ids: [(InvalidIdError, &[u8]); _] = [
            ...
            (InvalidIdError::TooLong, b"1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890"),
            (InvalidIdError::TooLong, b".2345678901234567890123456789012345678901234567890123456789012"),
        ];
