...
echo 'NetHSM rulez!' | openssl dgst -sha512 -binary | pkcs11-tool --module ./target/debug/libnethsm_pkcs11.so  -v  \
  --sign --mechanism ECDSA --output-file _data.sig --signature-format openssl --id $HEXID

echo 'NetHSM rulez!' | openssl dgst -keyform PEM -sha512 -verify _public.pem -signature _data.sig
