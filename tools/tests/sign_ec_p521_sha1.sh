...
pkcs11-tool --module ./target/debug/libnethsm_pkcs11.so  -v  \
  --sign --mechanism ECDSA-SHA1 --output-file _data.sig --signature-format openssl --id $HEXID --input-file _data

cat _data | openssl dgst -sha1 -binary | openssl pkeyutl -verify -sigfile _data.sig -inkey _public.pem -keyform PEM -pubin 
